...
- Поддержка подписок (`Subscribe URL`) для режимов:
  ...
  - `Selector`
  - `URLTest`
- Режим по умолчанию после установки: `Configuration Type = URLTest`.
...
- `Mobile import key (masked)` (ключ показывается только частично)
- готовый шаблон URL для кнопки
- команду для просмотра полного ключа прямо на роутере:
  - `uci get podkop.settings.mobile_import_key`

Формат ссылки для кнопки в подписке:

```text
http://ROUTER_IP/cgi-bin/podkop-import-subscription?key=YOUR_KEY&mode=urltest&url=URL_ENCODED_SUBSCRIPTION
```
